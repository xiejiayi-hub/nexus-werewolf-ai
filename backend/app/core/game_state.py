...
class GameStateMachine:
    ...
    def record_guardian_protect(self, target_id: int):
        if self.phase == GamePhase.NIGHT_GUARDIAN:
            self.guardian_target = target_id

    def resolve_night(self):
        killed = self.resolve_wolf_kill()

        if killed and killed == self.guardian_target:
            print(f"🛡️ 守卫守住了 {killed}，本轮无人死亡")
            self.killed_target = None
        else:
            self.killed_target = killed
    ...
    def add_speech(self, player_id: int, speech: str):
